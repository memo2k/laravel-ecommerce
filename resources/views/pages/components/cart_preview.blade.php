...
<div class="invisible absolute right-0 top-full z-20 mt-2 w-[27rem] translate-y-1 rounded-lg border border-slate-200 bg-white p-4 text-slate-800 opacity-0 shadow-lg transition-all duration-150 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
    <div class="mb-2 flex items-center justify-between">
        ...
        <span class="text-xs text-slate-500">{{ $cartTotalProducts ?? 0 }} items</span>
    </div>
    ...
    @else
        <ul class="max-h-72 space-y-2 overflow-y-auto pr-1">
            @foreach ($cartData->take(4) as $item)
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <button type="submit" class="text-sm font-semibold leading-none text-slate-400 transition-colors hover:text-red-500" aria-label="Remove item">
                                    x
                                </button>
                            </div>
                            <div class="mt-2 flex items-center justify-between">
                                <div class="inline-flex items-center rounded border border-slate-200">
                                    <button type="submit" class="px-2 py-1 text-sm text-slate-700 hover:bg-slate-100" aria-label="Decrease quantity">
                                        -
                                    </button>
                                    <span class="min-w-7 border-x border-slate-200 px-2 py-1 text-center text-[11px] text-slate-700">
                                        {{ $item['quantity'] }}
                                    </span>
                                    <button type="submit" class="px-2 py-1 text-sm text-slate-700 hover:bg-slate-100" aria-label="Increase quantity">
                                        +
                                    </button>
                                </div>
        ...
        @if (count($cartData) > 4)
            <p class="mt-2 text-[11px] text-slate-500">
                +{{ count($cartData) - 4 }} more item(s) in cart
            </p>
        @endif
        ...
        <div class="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
            ...
            <span class="font-semibold text-slate-900">${{ $cartTotalPrice ?? '0.00' }}</span>
        </div>
    @endif
